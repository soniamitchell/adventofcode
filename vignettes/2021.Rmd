...
while (nrow(unfinished) != 0) {
  ...
  test <- apply(unfinished, 1, function(x) {
    ...
    sum(islower)}) 
  # Remove routes where small caves were more than one cave was visited more 
  # than once
  index <- which(test > 1)
  if (length(index) > 1)
  ...
  complete <- dplyr::bind_rows(
    dplyr::filter(unfinished, dplyr::all_of(get(last_step)) == "end"), complete)
  
  # Remove completed rows from `unfinished`
  unfinished <- dplyr::filter(unfinished, 
                              dplyr::all_of(get(last_step)) != "end")
}
...
```{r day13-1}
# Read in data
dat <- scan(file.path(here(), "inst", "2021", "day13.txt"))
...
```{r day14-1}
# Read in data
dat <- scan(file.path(here(), "inst", "2021", "day14.txt"))
