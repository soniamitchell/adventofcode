...
# Tidy up data
dat <- dat %>% 
  dplyr::mutate(from = dplyr::case_when(V1 == "start" ~ V1,
                                        V2 == "start" ~ V2,
                                        V1 == "end" ~ V2,
                                        V2 == "end" ~ V1,
                                        T ~ V1),
                to = dplyr::case_when(V2 == "start" ~ V1,
                                      V1 == "start" ~ V2,
                                      V1 == "end" ~ V1,
                                      V2 == "end" ~ V2,
                                      T ~ V2)) %>% 
...
# Get name of adjacent caves
get_next <- function(cave, dat) {
  to <- dat %>% dplyr::filter(from == cave, to != "start")
  from <- dat %>% dplyr::filter(to == cave, from != "start")
  unique(c(to$to, from$from))
}
...
while (length(unfinished_routes) > 0) {
  ...
  if (this_cave == "end") {
    completed_routes <- c(completed_routes, list(this_route))
    unfinished_routes <- unfinished_routes[-1]
    next
  }
  
  # Record next step(s) of the route if:
  next_steps <- get_next(this_cave, dat)
  add_these <- lapply(next_steps, function(x) {
    if (grepl("^[[:upper:]]+$", x)) { # the cave is large
      c(this_route, x)
    } else { # or the cave is small 
      duplicates <- c(x, this_route)[duplicated(c(x, this_route))]
      if (!(x %in% this_route)) c(this_route, x) 
    }
  })
  # Tidy up `add_these`
  remove_null <- lapply(add_these, is.null) %>% unlist() %>% which()
  if (length(remove_null) > 0) add_these <- add_these[-remove_null]
...
Rather than replacing the small caves test:
...
```{r, eval=FALSE}
duplicates <- c(x, this_route)[duplicated(c(x, this_route))]
test <- vapply(vector, function(x) grepl("^[[:lower:]]+$", x), logical(1))
if (sum(test) <= 1) c(this_route, x) 
```

the code needs re-written for computational efficiency.
