...
compare_heights <- function(dat, row, column) {
  ...
  # If `height` is lower than all `adjacent_points`, return risk level
...
```{r day9-2}
check_depth <- function(dat, row, column) {
  if (dat[row, column] < 9) 
...
check_adjacent <- function(dat, row, column) {
  ...
  # Above
  if (row != 1) 
    adjacent_points <- rbind(adjacent_points, check_depth(dat, row - 1, column))
  # Right
  if (column != ncol(dat)) 
    adjacent_points <- rbind(adjacent_points, check_depth(dat, row, column + 1))
  # Below
  if (row != nrow(dat)) 
    adjacent_points <- rbind(adjacent_points, check_depth(dat, row + 1, column))
  # Left
...
prod(tail(sort(results), 3)) # 148824
```
