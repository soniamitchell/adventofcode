...
```{r day2-1}
# Read in data
dat <- read.table(here("inst", "2021", "day2.txt"))
...
raw_numbers <- scan(here("inst", "2021", "day4.txt"), 
                    what = "character", n = 1) 
...
```{r day5-1}
# Read in data
dat <- read.table(here("inst", "2021", "day5.txt"))
...
```{r day6-1}
# Read in data
dat <- scan(here("inst", "2021", "day6.txt"), 
            what = "character", sep = ",") %>% 
  as.numeric()
...
```{r day7-1}
# Read in data
dat <- scan(here("inst", "2021", "day7.txt"), 
            what = "character", sep = ",") %>% 
  as.numeric()
...
```{r day8-1}
# Read in data
dat <- scan(here("inst", "2021", "day8.txt"), 
            what = "character", sep = "\n")
...
dat <- readLines(here("inst", "2021", "day9.txt")) %>% 
  strsplit("") %>% 
  do.call(rbind, .) %>% 
  apply(1, as.numeric)
...
dat <- readLines(here("inst", "2021", "day10.txt")) 
...
dat <- scan(here("inst", "2021", "day11.txt"), 
            what = "character") %>% 
  strsplit("") %>% 
  do.call(rbind, .) %>% 
  apply(1, as.numeric)
...
```{r day12-1}
# Read in data
dat <- read.table(here("inst", "2021", "day12.txt"), sep = "-")
...
dat <- read.table(here("inst", "2021", "day13.txt"), sep = "-")
...
template <- scan(here("inst", "2021", "day14.txt"), 
                 what = "character", n = 1) 
...
Link to questions [here](https://adventofcode.com/2021/day/19)
...
res <- lapply(seq_len(nrow(combinations)), function(x) {
  ...
  a <- scanners %>% 
    dplyr::filter(beacon == first) %>% 
    dplyr::select(-beacon) %>% unlist()
  b <- scanners %>% 
    dplyr::filter(beacon == second) %>% 
    dplyr::select(-beacon) %>% unlist()
  manhattan(a, b)
}) %>% unlist()
...
Link to questions [here](https://adventofcode.com/2021/day/20)
